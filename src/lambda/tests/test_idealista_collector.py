...
from idealista_listings_collector import (
    IdealistaAPIError,
    SearchConfig,
    get_oauth_token,
    get_secret,
...
def mock_aws_clients():
    ...
    with patch('idealista_listings_collector.s3_client') as mock_s3, \
         patch('idealista_listings_collector.secrets_client') as mock_secrets:
...
class TestProcessOperation:
    ...
    def test_process_operation_test_mode(self, mock_query, mock_env_vars, mock_aws_clients):
        ...
        config = SearchConfig()
        result = process_operation('sale', config, 'test-key', 'test-secret', max_pages=1)
    ...
    @patch('idealista_listings_collector.query_api')
    def test_process_operation_multiple_pages(self, mock_query, mock_env_vars, mock_aws_clients):
...
class TestLambdaHandler:
    ...
    def test_lambda_handler_test_mode(self, mock_get_secret, mock_process, mock_env_vars):
    ...
    @patch('idealista_listings_collector.process_operation')
    @patch('idealista_listings_collector.get_secret')
    def test_lambda_handler_normal_mode(self, mock_get_secret, mock_process, mock_env_vars):
